...
export const LogInPage = () => {
    ...
    const registerHandler = async () => {
        try {
            auth.socket.emit("api/register", {...form}, callback=>{
                console.log("Data", callback.message)
                auth.login(callback.token, callback.userId)
            })
        } catch (e) {
            console.log(`registerHandlerError ${e.message}`)
        }
    }

    const loginHandler = async () => {
        try {
            auth.socket.emit ("api/login", {...form}, callback=>{
                console.log("Data", callback)
                auth.login(callback.token, callback.userId)
            })
        } catch (e) {
            console.log(`loginHandlerError ${e.message}`)
        }
    }
